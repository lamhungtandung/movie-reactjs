...
export default function CenteredTabs({
  ...
  useEffect(() => {
    if (onClickBtnMuave !== 0) {
      // không scroll khi mới load
      scroll("TapMovieDetail");
    }
  }, [croll]); // khi nhấn mua ve đã hoàn thành => tap 0 thì scroll
  ...
  const handleLike = (id) => {
    ...
    if (commentUserLiked.userLikeThisComment.includes(currentUser.email)) {
      // xóa user khỏi ds liked comment => trừ số like
        ...
        <DialogActions className="justify-content-center flex-column px-4">
          {warningtext && (
            <DialogContentText className="text-danger">
              Phim khiến bạn cảm thấy tuyệt vời chứ? Hãy chia sẻ vố chúng tôi! (Gõ trên 60 kí tự.)
            </DialogContentText>
          )}
          <Button
